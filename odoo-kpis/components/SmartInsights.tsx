  ...
  return (
    <Card className="border-slate-200 dark:border-slate-700/60 bg-white/90 dark:bg-slate-900/80 backdrop-blur-sm">
      <CardHeader className="pb-3 flex flex-row items-center gap-2">
        ...
        <div>
          <CardTitle className="text-sm font-semibold text-slate-800 dark:text-slate-200">Smart Insights</CardTitle>
      ...
      <CardContent>
        {loading ? (
          <div className="grid gap-3 sm:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-24 rounded-lg bg-slate-200 dark:bg-slate-800/50 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-3">
