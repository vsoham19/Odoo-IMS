...
export default function DashboardPage() {
  ...
  const pendingDeliveries = data.deliveries.filter(
    (d) => d.status?.toLowerCase() !== "completed" && d.status?.toLowerCase() !== "done"
  ).length;

  const pendingReceipts = data.receipts.filter(
    (r) => r.status?.toLowerCase() !== "completed" && r.status?.toLowerCase() !== "done"
  ).length;
      ...
      <div className="relative mx-auto max-w-screen-2xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            {error && (
              <span className="rounded-full bg-rose-500/10 border border-rose-500/20 px-3 py-1 text-xs text-rose-400">
                {error}
              </span>
            )}
            <Button
              ...
              disabled={refreshing}
              className="border-slate-700 bg-slate-900 text-slate-300 hover:bg-slate-800 hover:text-slate-100 gap-2"
            >
